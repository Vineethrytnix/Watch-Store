    ...
    while ($row = mysqli_fetch_array($exe)) {
        ...
        <form id="watchServiceForm" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="watchBrand" placeholder="Enter Watch Brand" value="<?php echo $row['wbrand'] ?>" name="watchBrand" class="form-control"
                    required>
            </div>
            <div class="form-group">
                <label for="watchModel">Watch Model:</label>
                <input type="text" id="watchModel" name="watchModel" value="<?php echo $row['model'] ?>" placeholder="Enter Watch Model" class="form-control"
                    required>
            </div>
            <div class="form-group">
            ...
            <center><button type="submit" name="submit">Update Service</button></center>
        </form>
    <?php } ?>
</div>
...
if (isset($_REQUEST['submit'])) {
    ...
    if (move_uploaded_file($tempname, $folder)) {
        $qryReg = "UPDATE `services` SET `wbrand`='$watchBrand',`model`='$watchModel',`ssimage`='$filename',`service`='$serviceType' WHERE `service_id`='$ser_id'";
        if ($conn->query($qryReg) == TRUE) {
            echo "<script>alert('Service Updated'); window.location = 'add_service.php';</script>";
        } else {
