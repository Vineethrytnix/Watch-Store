...
if (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($tempname, $folder)) {
        ...
        if ($fetchData['cnt'] > 0) {
            ...
            $qryReg = "INSERT INTO `service_centre`(`sname`,`semail`,`sphone`,`simage`,`saddress`)VALUES('$Name','$Email','$Phone','$filename','$Address')";
            $qryLog = "INSERT INTO `login` (`rid`, `email`, `password`, `type`) VALUES((select max(`s_id`) from `service_centre`), '$Email', '$Password', 'SERVICE CENTRE')";


            if ($conn->query($qryReg) == TRUE && $conn->query($qryLog) == TRUE) {
                echo "<script>alert('Registration Success'); window.location = 'login.php';</script>";
            } else {
                echo "<script>alert('Registration Failed'); window.location = 'service_centre.php';</script>";
            }
        }
    } else {
